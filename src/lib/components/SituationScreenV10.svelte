<script lang="ts">
	...
	import type { Situation } from '$lib/stores/game-v10.svelte';

	const game = getGameStateV10();

	let canContinue = $derived(game.situation !== null);
</script>
...
<main class="min-h-svh flex items-center justify-center px-5 py-10">
	<div class="w-full max-w-sm md:max-w-2xl mx-auto">
		...
		<p class="text-lg md:text-xl text-gray-800 text-center mb-8">
			Kies je situatie
		</p>
