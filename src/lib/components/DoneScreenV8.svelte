<script lang="ts">
	import { getGameStateV8 } from '$lib/stores/game-v8.svelte';
	import { questionGroupsV8 } from '$lib/data/questions-v8';

	const game = getGameStateV8();

	let totalVariants = $derived(
		questionGroupsV8.reduce((sum, g) => sum + g.variants.length, 0)
	);
</script>
		...
		<div class="mb-8">
			{#if game.submissionStatus === 'submitting'}
				<div class="mx-auto mb-6 flex h-20 w-20 items-center justify-center rounded-full bg-teal-100">
					<svg class="h-10 w-10 text-teal-600 animate-spin" fill="none" viewBox="0 0 24 24">
			...
			{:else if game.submissionStatus === 'queued'}
				...
				<h1 class="text-3xl font-bold text-gray-900 mb-3">Bedankt!</h1>
				<p class="text-lg text-amber-600 mb-2">
					Je selectie is lokaal opgeslagen en wordt automatisch verzonden zodra de verbinding hersteld is.
				</p>
			{:else}
				...
				<h1 class="text-3xl font-bold text-gray-900 mb-3">Bedankt!</h1>
				<p class="text-lg text-gray-500 mb-2">Je selectie is succesvol verzonden.</p>
			{/if}
		</div>

		<div class="rounded-2xl bg-gray-50 p-6 mb-8">
			<p class="text-3xl font-bold text-teal-600">{game.totalLiked}</p>
			<p class="text-sm text-gray-500">van {totalVariants} vragen als "goede vraag" geselecteerd</p>
		</div>
